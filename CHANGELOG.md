# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## 1.4.0 - 2026-01-07

### Added

- Add AI watermark metadata @ marcelklehr [#40](https://github.com/nextcloud/text2speech_kokoro/pull/40)
- Support trigger for taskprocessing @marcelklehr [#33](https://github.com/nextcloud/text2speech_kokoro/pull/33)

### Fixed

- Add missing packages to Dockerfile @lukasdotcom [#48](https://github.com/nextcloud/text2speech_kokoro/pull/48)

## 1.3.0 - 2025-12-01

### Added

- Use custom repo for model @lukasdotcom [#36](https://github.com/nextcloud/text2speech_kokoro/pull/36)
- Support trigger for taskprocessing @marcelklehr [#33](https://github.com/nextcloud/text2speech_kokoro/pull/33)

### Fixed

- Improve pylint settings @lukasdotcom [#35](https://github.com/nextcloud/text2speech_kokoro/pull/35)

## 1.2.0 - 2025-10-20

### Added

- CUDA or Nvidia GPU support #25
- Nextcloud 33 support #30

### Fixed

- Improve error handling #24

## 1.1.1 - 2025-07-16

### Fixed

- Typo in readme for the ethical ai rating #13
- Use the correct task type even when restarted #14

## 1.1.0 - 2025-06-18

### Added

- Initial release
